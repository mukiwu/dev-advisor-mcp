...
**✨ 新功能：整合 Baseline 狀態查詢** - 基於 [web.dev/baseline](https://web.dev/baseline) 判斷 Web API 是否可在所有核心瀏覽器中安全使用。
...
### 📊 Baseline 狀態查詢（新增）
- 整合 [web.dev/baseline](https://web.dev/baseline) 資料
...
| 參數 | 類型 | 必填 | 說明 |
|------|------|------|------|
| `feature` | string | ✅ | Web API 功能名稱，如 `"fetch"`、`"css-grid"`、`"IntersectionObserver"` |
...
檢查 CSS Grid 的瀏覽器相容性
fetch API 是否可安全使用？
IntersectionObserver 是否已達到 Baseline？
```
